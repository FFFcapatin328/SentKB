...
def train(train_triplets, model, batch_size, split_size, negative_sample, reg_ratio, num_entities, num_relations):
    ...
    if torch.cuda.is_available():
        ...
        train_data.to(torch.device('cpu'))
...
if __name__ == '__main__':
    ...
    all_seeds = pickle.load(open('kg/all_seeds.pkl', 'rb'))
    relation_map = pickle.load(open('kg/relation_map.pkl', 'rb'))
    unique_nodes_mapping = pickle.load(open('kg/unique_nodes_mapping.pkl', 'rb'))
    concept_graphs = pickle.load(open('kg/concept_graphs.pkl', 'rb'))
    train_triplets = np.load(open('kg/triplets.np', 'rb'), allow_pickle=True)
    ...
    model.load_state_dict(torch.load('weights/model_epoch' + str(CHECKPOINT) +'.pt'))
    model.to('cpu') # need more GPU memory
    model.eval()
    if not os.path.exists("graph_features"):
        os.mkdir("graph_features")
    for split in ['train', 'dev', 'test']:
        resPath = 'graph_features/sf_' + split + '_' + str(CHECKPOINT) + '_5000.np'
        if not os.path.exists(resPath):
            sf = sentence_features(model, split, all_seeds, concept_graphs, relation_map, unique_nodes_mapping)
            np.ndarray.dump(sf, open(resPath, 'wb'))
