...
def get_seeds(max_words,type = 'all'):
    ...
    if type == 'all':
        data_dir1 = 'data/VAST/vast_train.csv'
        data_dir2 = 'data/VAST/vast_test.csv'
        data_dir3 = 'data/VAST/vast_dev.csv'
        data1 = pd.read_csv(data_dir1)
        data2 = pd.read_csv(data_dir2)
        data3 = pd.read_csv(data_dir3)
        data = pd.concat([data1,data2,data3],axis=0)
    else:
        data_dir1 = 'data/VAST/vast_'+type+'.csv'
...
if __name__ == '__main__':
    ...
    G, G_reverse, concept_map, relation_map = conceptnet_graph('data/conceptnet_english.txt')
    ...
    if not os.path.exists("kg"):
        os.mkdir("kg")
    pickle.dump(all_seeds, open('kg/all_seeds.pkl', 'wb'))
    pickle.dump(concept_map, open('kg/concept_map.pkl', 'wb'))
    pickle.dump(relation_map, open('kg/relation_map.pkl', 'wb'))
    pickle.dump(unique_nodes_mapping, open('kg/unique_nodes_mapping.pkl', 'wb'))
    pickle.dump(word_index, open('kg/word_index.pkl', 'wb'))
    pickle.dump(concept_graphs, open('kg/concept_graphs.pkl', 'wb'))
    
    np.ndarray.dump(triplets, open('kg/triplets.np', 'wb'))        
    print ('Completed.')